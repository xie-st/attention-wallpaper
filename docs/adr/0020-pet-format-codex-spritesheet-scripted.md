...
- **Article switching — sequential + end-of-article hold + double-click to advance**: when the pet reaches the article's end, it enters the `end-of-article` state (row 5) and holds there. The next article does not auto-advance. Double-clicking the pet in the `end-of-article` state advances to the next article (in import-timestamp order, looping at the last back to the first). This reuses the double-click gesture (which in `walk-down`/`walk-up` states means rewind) under a different state — same gesture, context-dependent semantics: walking → rewind; at-end → next article.
- Article ordering is purely by import timestamp; no article-level priority in MVP (consistent with the user's "no Priority" statement during N1.1).
